...
class DecksController {
  static async returnsDecks(req, res) {
    const decks = await Deck.find({})
      ...
      .lean();
    if (!decks.length)
      return res.status(404).send({ error: "No decks were found" });
  ...
  static async registerDeck(req, res) {
    try {
      ...
      res.status(201).send({ success: `Deck ${req.body.name} created`, deck });
    } catch (error) {
  ...
  static async deleteDeck(req, res) {
    ...
    try {
      ...
      if (deletedDeck) res.status(200).send({ success: `Baralho apagado` });
        ...
        res.status(403).send({
          error: "You dont't have permisions to delete this resource"
        });
    } catch (error) {
      res.status(400).send(error);
    }
  ...
  static async updateDeck(req, res) {
    ...
    try {
      ...
      if (updatedDeck) {
        res.status(200).send({
          success: `Deck ${req.body.name} updated`,
          deck: updatedDeck
        });
      } else {
        res
          .status(403)
          .send({ error: "You don't have permisions to update this deck" });
      }
    } catch (error) {
      res.status(400).send(error);
    }
